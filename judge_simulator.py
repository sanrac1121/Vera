...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        if not self.api_key:
            raise ValueError("GEMINI_API_KEY is empty! Please paste your key into LLM_API_KEY in judge_simulator.py.")
        ...
        for m in models_to_try:
            ...
            try:
                resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
                data = json.loads(resp.read().decode("utf-8"))
                self.model = m  # set working model
                return data["candidates"][0]["content"]["parts"][0]["text"]
            except urlerror.HTTPError as e:
                err_body = e.read().decode("utf-8", errors="ignore")
                last_error = f"HTTP {e.code} ({m}): {err_body}"
                if e.code == 429:
                    time.sleep(5)  # wait for free tier rate limit reset
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _fallback_score(self, action: Dict) -> ScoreResult:
        """Enhanced heuristic rule-based scoring when LLM is offline/rate-limited."""
        body = action.get("body", "")
        body_lower = body.lower()
        cta = str(action.get("cta", "")).lower()
        rationale = action.get("rationale", "").lower()

        # 1. Specificity (numbers, %, ₹ prices, dates, citations)
        nums = len(re.findall(r'\d+', body))
        has_price = "₹" in body or "rs" in body_lower or "rupees" in body_lower
        has_percent = "%" in body or "percent" in body_lower
        has_citation = any(c in body_lower for c in ["jida", "page", "trial", "study", "dci", "report"])
        spec_score = min(10, 4 + (nums * 2) + (3 if has_price else 0) + (2 if has_percent else 0) + (2 if has_citation else 0))

        # 2. Category Fit (titles, clinical terms, vertical vocabulary)
        cat_fit = 8 if any(w in body_lower for w in ["dr.", "doctor", "clinic", "salon", "restaurant", "cafe", "gym", "pharmacy", "thali", "haircut", "cleaning", "recall", "appointment"]) else 6

        # 3. Merchant Fit (personalization, Hinglish code-mixing, owner name)
        merch_fit = 8 if any(w in body_lower for w in ["hi ", "hello ", "aap", "aapka", "apke", "shukriya", "bol", "rahi", "hai", "hain"]) else 7

        # 4. Decision Quality / Trigger Relevance (clear reason for message)
        dec_qual = 9 if len(rationale) > 10 and "error" not in rationale else (8 if len(body) > 30 else 5)

        # 5. Engagement Compulsion (CTA clarity, loss aversion, curiosity)
        eng_score = 9 if ("yes" in cta or "reply" in body_lower or "choice" in cta or "stop" in cta or "1" in body_lower) else 6

        return ScoreResult(
            specificity=spec_score,
            specificity_reason=f"Found {nums} numerical fact(s), price/citation signals",
            category_fit=cat_fit,
            category_fit_reason="Vertical domain vocabulary matches context rules",
            merchant_fit=merch_fit,
            merchant_fit_reason="Personalized owner outreach & language tone match",
            decision_quality=dec_qual,
            decision_quality_reason="Anchored on trigger event context payload",
            engagement_compulsion=eng_score,
            engagement_reason="Single binary CTA with low-friction ask",
            hint="Rule-based heuristic evaluation executed successfully"
        )
...
def main():
    ...
    try:
        ...
        print_warn(f"LLM connection rate-limited or unavailable: {e}")
        print_info("Switching to Heuristic Rule-Based Evaluator Mode...")
        class HeuristicMockProvider(LLMProvider):
            def name(self) -> str:
                return "Rule-Based Evaluator (Offline)"
            def complete(self, prompt: str, system: str = None) -> str:
                raise Exception("Rule-Based Fallback")
